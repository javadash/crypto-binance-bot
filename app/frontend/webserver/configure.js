...
app.post('/webhook', async (req, res) => {
  ...
  if (incomingPassphrase !== expectedPassphrase) {
    ...
    return res.status(403).json({ success: false, message: 'Invalid passphrase' });
  }
  ...
  await cache.hset('tradingview', 'latest-alert', JSON.stringify(alert), 3600); // Cache for 1 hour
